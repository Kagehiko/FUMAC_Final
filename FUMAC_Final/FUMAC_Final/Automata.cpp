...
void Automata::removeNonCoaccessibleStates(std::ostream& stream) {
	...
	stream << std::endl << "Coaccessible states: {";
	...
	stream << "}" << std::endl;
	stream << "Deleting all non-coaccessible states..." << std::endl;

	keepStates(coaccessible_states, stream);
}

void Automata::trim(std::ostream& stream) {
	this->removeNonAccessibleStates(stream);
	this->removeNonCoaccessibleStates(stream);
}